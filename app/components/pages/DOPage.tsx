...
export default function DOPage({ w }: Props) {
  if (w.selectedDO) {
    const selectedDO = w.selectedDO;
    const items = selectedDO.do_items || [];
    const shipments = (selectedDO.pengiriman || []).sort((a, b) => a.tahap - b.tahap);
    const totalShipped = shipments.reduce((s, p) => s + p.jumlah_zak, 0);
    const remaining = selectedDO.total_zak - totalShipped;
    return (
      <div>
          ...
          <div>
            <div className="card" style={{ marginBottom: '18px' }}>
              <div className="detail-header">
                ...
                <div>
                  <div className="detail-title" style={{ fontFamily: 'monospace' }}>{selectedDO.no_do}</div>
                  <div className="detail-sub">{fmtDate(selectedDO.tanggal)}</div>
                </div>
                <div style={{ marginLeft: 'auto' }}>
                  <span className={`badge ${statusDOBadge[selectedDO.status]}`} style={{ fontSize: '13px', padding: '4px 12px' }}>{statusDOLabel[selectedDO.status]}</span>
                </div>
              </div>
              <div className="info-row"><div className="info-key">Toko</div><div className="info-val font-bold">{selectedDO.toko?.nama || '—'}</div></div>
              <div className="info-row"><div className="info-key">Sopir / Armada</div><div className="info-val">{selectedDO.angkutan?.nama_sopir || '—'} · {selectedDO.angkutan?.no_polisi || '—'}</div></div>
              <div className="info-row"><div className="info-key">Total Zak</div><div className="info-val font-bold text-accent">{fmt(selectedDO.total_zak)} Zak</div></div>
              ...
              <div className="info-row"><div className="info-key">Total Pallet</div><div className="info-val">{selectedDO.total_pallet}</div></div>
              {selectedDO.catatan && <div className="info-row"><div className="info-key">Catatan</div><div className="info-val text-muted">{selectedDO.catatan}</div></div>}
            </div>
            <div className="card" style={{ marginBottom: '18px' }}>
            ...
            {selectedDO.status !== 'selesai' && selectedDO.status !== 'batal' && (
              <div className="flex-row" style={{ gap: '10px', flexWrap: 'wrap' }}>
                <button className="btn btn-blue" onClick={() => { w.setPgDoId(selectedDO.id); w.setModalPengiriman(true); }}>+ Tambah Tahap Pengiriman</button>
                <button className="btn btn-purple" onClick={() => { w.setPlTokoId(selectedDO.toko_id); w.setPlDoId(selectedDO.id); w.setModalPallet(true); }}>+ Catat Pallet Keluar</button>
                {selectedDO.status === 'proses' && (
                  <button className="btn btn-success" onClick={async () => {
                    await w.supabase.from('delivery_order').update({ status: 'selesai' }).eq('id', selectedDO.id);
                    if (selectedDO.angkutan_id) await w.supabase.from('angkutan').update({ status: 'tersedia' }).eq('id', selectedDO.angkutan_id);
                    await w.supabase.from('audit_log').insert({
                      user_id: w.user?.id, tabel: 'delivery_order', aksi: 'UPDATE',
                      ringkasan: `DO ${selectedDO.no_do} selesai`,
                    });
                    w.triggerToast(`${selectedDO.no_do} ditandai selesai`);
                    w.fetchAll();
                ...
                {(w.user?.role === 'admin' || w.user?.role === 'superadmin') && (
                  <button className="btn btn-ghost" onClick={async () => {
                    ...
                    await w.supabase.from('delivery_order').update({ status: 'batal' }).eq('id', selectedDO.id);
                    await w.supabase.from('audit_log').insert({
                      user_id: w.user?.id, tabel: 'delivery_order', aksi: 'UPDATE',
                      ringkasan: `DO ${selectedDO.no_do} dibatalkan`,
                    });
                    w.triggerToast(`${selectedDO.no_do} dibatalkan`, 'error');
                    w.fetchAll();
                    ...
                    <div key={pg.id} className="timeline-item">
                      ...
                      <div className="timeline-body">
                        ...
                        <div className="text-xs text-muted" style={{ marginTop: '3px' }}>
                          Supir Tahap: {pg.angkutan?.nama_sopir || selectedDO.angkutan?.nama_sopir || '—'}{pg.angkutan?.no_polisi ? ` · ${pg.angkutan.no_polisi}` : selectedDO.angkutan?.no_polisi ? ` · ${selectedDO.angkutan.no_polisi}` : ''}
                        </div>
                        ...
                        <div className="flex-row" style={{ marginTop: '8px', gap: '6px' }}>
                          ...
                          {pg.status === 'persiapan' && (
                            <button className="btn btn-sm" style={{ background: 'var(--warn)', color: '#000', padding: '3px 9px', fontSize: '11px' }} onClick={async () => {
                              ...
                              await w.supabase.from('audit_log').insert({
                                ...
                                ringkasan: `Tahap ${pg.tahap} ${selectedDO.no_do} berangkat (${fmt(pg.jumlah_zak)} zak)`,
                              });
                          ...
                          {pg.status === 'jalan' && (
                            <button className="btn btn-success btn-sm" style={{ padding: '3px 9px', fontSize: '11px' }} onClick={async () => {
                              ...
                              await w.supabase.from('audit_log').insert({
                                user_id: w.user?.id, tabel: 'pengiriman', aksi: 'UPDATE',
                                ringkasan: `Tahap ${pg.tahap} ${selectedDO.no_do} tiba (${fmt(pg.jumlah_zak)} zak)`,
                              });
